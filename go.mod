...
// This replace block mirrors github.com/argoproj/argo-cd/v2@v2.14.13's own
// go.mod exactly. Replace directives in a dependency are ignored once it's
// not the main module, so its carefully pinned k8s.io/* matrix (needed
// because it vendors k8s.io/kubernetes transitively via gitops-engine) has
// to be replicated here or MVS drifts individual modules out of sync with
// each other, breaking structured-merge-diff v4/v6 and kube-openapi/yaml
// compatibility in ways that only surface as obscure internal type errors.
replace (
	...
	sigs.k8s.io/structured-merge-diff/v4 => sigs.k8s.io/structured-merge-diff/v4 v4.4.4-0.20241211184406-7bf59b3d70ee
